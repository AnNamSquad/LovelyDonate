package org.simpmc.lovelypay.menu;
    ...
    private CompletableFuture<List<PaymentRecord>> fetchPaymentRecordsAsync(Context context) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            if (context.getInitialData() == null) { // TODO: should be async ?
                spPlayer = LPPlugin.getService(DatabaseService.class).getPlayerService().findByUuid(context.getPlayer().getUniqueId());
            } else {
                spPlayer = LPPlugin.getService(DatabaseService.class).getPlayerService().findByName((String) context.getInitialData());
            }
